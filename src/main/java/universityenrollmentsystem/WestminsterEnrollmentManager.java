...
public class WestminsterEnrollmentManager implements EnrollmentManager{
    ...
    public boolean runMenu() {
        ...
        System.out.println("To Search by prefix of ID, press 4");
    ...
    public void searchPeopleByIDPrefix(){
        System.out.println("Search for people by prefix.");
        //Asking for input
        Scanner s = new Scanner (System.in);
        ...
        //searching for the things and storing in temporary arrayList
        ArrayList<Person> searchedPeople = new ArrayList<Person>();

        for(Person person: personList){
            if(person.getID().substring(0,3).equals(prefix)){
                searchedPeople.add(person);
            }
        }
        if(searchedPeople==null){
            System.out.println("There are no people in the system matching this prefix.");
        }
    ...
    public void printListSortedByBirthdateAndID(){
        //create a copy of person list
        ArrayList<Person> sortedPeople = new ArrayList<Person>();

        //list for null people
        ArrayList<Person> nullDobPeople = new ArrayList<Person>();

        //putting people into Sorted list
        for(Person person : personList){
            if(person.getDob()==null){
                nullDobPeople.add(person);
            }else{
                sortedPeople.add(person);
            }
        }
        //adding the null people to the end
        for(Person person :nullDobPeople){
            sortedPeople.add(person);
        }
